...
client.on(discord_js_1.Events.GuildMemberAdd, async (member) => {
    ...
    if (!guild || DB.isGuildEmpty(guild))
        return;
...
async function set_new_host(oldState, newState) {
    ...
    if (guildVoice.host.id === client.user.id) {
        const members = newState.channel?.members.filter(m => !m.user.bot) ?? new discord_js_1.Collection();
        const newHost = members.at(Math.floor(Math.random() * members.size));
...
function handle_error(err, opts = {}) {
    ...
    if (!_config_1.default.testing && client.is_ready) {
        ...
        if (nameCommand && interaction) {
            ...
            if (interaction.isCommand() && !interaction.isContextMenuCommand()) {
                ...
                if (sub_cmd_group_name)
                    nameCommand += ` ${sub_cmd_group_name}`;
                if (sub_cmd_name)
        ...
        if (nameCommand)
            ...
            error_str += '**Error occurred in bot!**\n';
...
function cleanup() {
    if (cleanedUp)
        return;
    cleanedUp = true;
    const content = '💨 My apologies, it appears my instruments are out of tune. ' +
